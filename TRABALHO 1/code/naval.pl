...
Em vez de substituir um diagnóstico único, adiciona um par [DataConsulta, Class] no início da lista de histórico.
Se o histórico for `nulo_historico`, cria-se a lista com uma entrada.
Atualiza o historial do paciente com [DataConsulta, Diagnostico]
